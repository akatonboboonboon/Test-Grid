...
const urls = {
  ...
  rapid: new URL("../app/subjects/subject-9/rapid/page.tsx", import.meta.url),
...
test("subject 9 exposes cards, normal practice, solved generation, random mock, A4 expected exam, and timed drill", async () => {
  const [page, rapid, expected] = await Promise.all([
    readFile(urls.page, "utf8"), readFile(urls.rapid, "utf8"), readFile(urls.expected, "utf8"),
  ]);
  for (const label of ["暗記帳", "通常演習", "その場で生成", "ランダム模試", "A4予想試験", "形式ガイド"]) assert.match(page, new RegExp(label));
  assert.match(page, /subjects\/subject-9\/rapid/);
  ...
  assert.match(rapid, /連続/);
  assert.match(rapid, /ランキング/);
  assert.match(rapid, /RANK_KEY/);
...
test("all problem surfaces render a blank linked diagram and solution surfaces opt into solution overlays", async () => {
  const [page, rapid, expected, diagrams, extraDiagrams] = await Promise.all([
    readFile(urls.page, "utf8"), readFile(urls.rapid, "utf8"), readFile(urls.expected, "utf8"),
    readFile(urls.diagrams, "utf8"), readFile(urls.extraDiagrams, "utf8"),
  ]);
  ...
  assert.match(rapid, /solution=\{status !== "active"\}/);
...
test("subject-9 implementation remains dedicated and does not require shared hub/data edits", async () => {
