...
import {
  ...
  type ThermodynamicsTopicId,
} from "../../thermodynamics-data";
...
type TestResult = { question: ThermodynamicsQuestion; response: string; correct: boolean };
type SavedTestSession = {
  version: 1;
  questionIds: string[];
  selectedTopics: ThermodynamicsTopicId[];
...
const PROGRESS_KEY = "test-grid:subject-4:progress:v1";
...
function parseNumericResponse(question: ThermodynamicsQuestion, value: string) {
...
function isCorrectAnswer(question: ThermodynamicsQuestion, response: string) {
...
function topicLabel(topicId: ThermodynamicsTopicId) {
  return THERMODYNAMICS_TOPICS.find((topic) => topic.id === topicId)?.shortTitle ?? topicId;
}

function formatLabel(question: ThermodynamicsQuestion) {
...
function restoreTestSession(): SavedTestSession | null {
  ...
  try {
    ...
    const selectedTopics = Array.isArray(parsed.selectedTopics)
      ? parsed.selectedTopics.filter((topic): topic is ThermodynamicsTopicId => typeof topic === "string" && KNOWN_TOPIC_IDS.has(topic as ThermodynamicsTopicId))
      : [];
...
function TopicFilter({
  ...
  legend: string;
  selected: ThermodynamicsTopicId[];
  onChange: (topics: ThermodynamicsTopicId[]) => void;
}) {
  function toggle(topicId: ThermodynamicsTopicId) {
...
function AnswerForm({
  ...
  idPrefix: string;
  question: ThermodynamicsQuestion;
  typedAnswer: string;
  selectedChoice: string;
  feedback: Feedback | null;
...
export default function ThermodynamicsSubjectPage() {
  ...
  const [cardTopics, setCardTopics] = useState<ThermodynamicsTopicId[]>([...ALL_TOPIC_IDS]);
  const [cardDeck, setCardDeck] = useState([...THERMODYNAMICS_FORMULAS]);
  ...
  const [testTopics, setTestTopics] = useState<ThermodynamicsTopicId[]>([...ALL_TOPIC_IDS]);
  ...
  const [testQuestions, setTestQuestions] = useState<ThermodynamicsQuestion[]>([]);
  ...
  const filteredCards = useMemo(
    () => THERMODYNAMICS_FORMULAS.filter((card) => cardTopics.includes(card.topic)),
    [cardTopics],
  );
  ...
  const totalMastered = THERMODYNAMICS_FORMULAS.filter((card) => progress[card.id] === "mastered").length;
  ...
  function changeCardTopics(nextTopics: ThermodynamicsTopicId[]) {
    setCardTopics(nextTopics);
    setCardDeck(THERMODYNAMICS_FORMULAS.filter((card) => nextTopics.includes(card.topic)));
    setCardIndex(0);
    setCardFlipped(false);
  }
  ...
  const pageStyle = { "--subject-accent": "#ff8c57", "--english-yellow": "#ff8c57" } as CSSProperties;
    ...
    <div className="app-frame generic-subject-page english-page statistics-page" style={pageStyle}>
      <header className="topbar">
        <Link className="brand" href="/" aria-label="9教科の一覧へ戻る">
          <span className="brand-mark hub-brand-mark" aria-hidden="true">T/F</span>
        ...
        <div className="header-actions statistics-header-actions">
          ...
          <Link className="outline-button header-link" href="/ranking/subject-4">公式ランキングテスト</Link>
      ...
      <main className="generic-subject-main english-main statistics-main">
        <nav className="subject-breadcrumb statistics-breadcrumb" aria-label="現在位置">
          <Link href="/">科目一覧</Link><span>/</span><strong aria-current="page">熱・流体力学</strong>
        </nav>
        ...
        <section className="generic-subject-hero english-hero statistics-hero" aria-labelledby="statistics-title">
          <div className="english-hero-copy statistics-hero-copy">
            <p><span>SUBJECT 04</span><span>COURSE-RANGE ONLY</span></p>
            <h1 id="statistics-title">熱・流体力学</h1>
            <small>{THERMODYNAMICS_RANGE_PAGES.length}枚の範囲と形式3の確認済み重複だけを学習し、公式・単位・線図・途中式を反復します。正式時間と満点は資料で未確認のため、模試は時間を自分で設定できます。</small>
          </div>
          <button className="english-hero-memory-button statistics-hero-card-button" type="button" onClick={() => changeMode("cards")}>
            ...
            <small>{THERMODYNAMICS_FORMULAS.length}枚を単元別に暗記 →</small>
          </button>
        </section>
        ...
        <section ref={workspaceRef} id="statistics-workspace" className="english-workspace statistics-workspace">
          <div className="workspace-tabs english-tabs statistics-tabs" role="tablist" aria-label="熱・流体力学の学習モード">
          ...
          {mode === "scope" && (
            <section className="english-guide-workspace statistics-scope-workspace" aria-labelledby="statistics-scope-title">
              <div className="english-panel-heading statistics-panel-heading">
                ...
                <p>テスト範囲ZIPの{THERMODYNAMICS_RANGE_PAGES.length}枚と、形式3のうち今回範囲と確認できた部分から整理した{THERMODYNAMICS_TOPICS.length}単元です。</p>
              </div>
            ...
            <section className="generic-test-workspace english-test-workspace statistics-practice-workspace" aria-labelledby="statistics-practice-title">
              ...
              {currentPracticeQuestion ? (
                <div className="english-test-active statistics-practice-active">
                  ...
                  {currentPracticeQuestion.diagram && <ThermodynamicsDiagram kind={currentPracticeQuestion.diagram} title="解答用線図" />}
            ...
            <section className="generic-test-workspace english-test-workspace statistics-test-workspace" aria-labelledby="statistics-test-title">
              ...
              {testPhase === "active" && currentTestQuestion && (
                <div className="english-test-active statistics-test-active">
                  ...
                  {currentTestQuestion.diagram && <ThermodynamicsDiagram kind={currentTestQuestion.diagram} title="解答用線図" />}
          ...
          {mode === "expected" && <ThermodynamicsExpectedExams />}
          ...
          {mode === "guide" && (
            <section className="english-guide-workspace statistics-guide-workspace" aria-labelledby="statistics-guide-title">
              ...
              <div className="english-guide-tip statistics-format-notice"><span>SOURCE BOUNDARY</span><p>形式1.jpg・形式2.pdfは形式だけを参照します。形式3.pdfは確認済みの範囲重複だけを学習内容に使い、それ以外は形式参考に限定します。正式時間・満点・配点は不明で、{THERMODYNAMICS_EXAM_SPEC.scoreLabel}はサイト上の練習設定です。</p></div>
              <div className="english-guide-grid statistics-guide-grid">
                {THERMODYNAMICS_EXAM_FORMATS.map((format, index) => (
              ...
              <div className="english-guide-tip statistics-exam-tip"><span>EXAM ROUTINE</span><p><b>既知量と単位をそろえる → 状態方程式・第一法則など使う式を書く → 符号規約を確認 → 代入 → 単位と有効数字を整える。</b>P-V・T-S・h-s線図と途中式を残して部分点を狙います。</p></div>
            </section>
          )}
        </section>
      ...
      <footer className="english-footer statistics-footer"><span>TEST//GRID</span><p>THERMODYNAMICS · USER-SET TIME · PRACTICE SCORE /100 · LINE 60</p><span>SUBJECT 04</span></footer>
    </div>
  );
}
