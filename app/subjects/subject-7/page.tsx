...
type TestResult = { question: StatisticsQuestion; response: string; correct: boolean };
type SavedTestSession = {
  version: 1;
  questionIds: string[];
  selectedTopics: StatisticsTopicId[];
...
const PROGRESS_KEY = "test-grid:subject-7:progress:v1";
...
const KNOWN_QUESTION_IDS = new Set(STATISTICS_QUESTIONS.map((question) => question.id));
...
function topicLabel(topicId: StatisticsTopicId) {
  return STATISTICS_TOPICS.find((topic) => topic.id === topicId)?.shortTitle ?? topicId;
}

function formatLabel(question: StatisticsQuestion) {
...
function restoreTestSession(): SavedTestSession | null {
  ...
  try {
    ...
    const selectedTopics = Array.isArray(parsed.selectedTopics)
      ? parsed.selectedTopics.filter((topic): topic is StatisticsTopicId => typeof topic === "string" && KNOWN_TOPIC_IDS.has(topic as StatisticsTopicId))
      : [];
...
function TopicFilter({
  ...
  legend: string;
  selected: StatisticsTopicId[];
  onChange: (topics: StatisticsTopicId[]) => void;
}) {
  function toggle(topicId: StatisticsTopicId) {
...
export default function StatisticsSubjectPage() {
  ...
  const [cardTopics, setCardTopics] = useState<StatisticsTopicId[]>([...ALL_TOPIC_IDS]);
  const [cardDeck, setCardDeck] = useState([...STATISTICS_FORMULAS]);
  const [cardIndex, setCardIndex] = useState(0);
  const [cardFlipped, setCardFlipped] = useState(false);

  const [practiceTopics, setPracticeTopics] = useState<StatisticsTopicId[]>([...ALL_TOPIC_IDS]);
  const [practiceDeck, setPracticeDeck] = useState([...STATISTICS_QUESTIONS]);
  ...
  const [testTopics, setTestTopics] = useState<StatisticsTopicId[]>([...ALL_TOPIC_IDS]);
  const [questionCountDraft, setQuestionCountDraft] = useState("20");
  const [testQuestions, setTestQuestions] = useState<StatisticsQuestion[]>([]);
  ...
  const filteredCards = useMemo(
    () => STATISTICS_FORMULAS.filter((card) => cardTopics.includes(card.topic)),
    [cardTopics],
  );
  ...
  const availableTestQuestions = useMemo(
    () => STATISTICS_QUESTIONS.filter((question) => testTopics.includes(question.topic)),
    [testTopics],
  );
  const currentTestQuestion = testQuestions[testIndex];
  const testScore = testResults.filter((result) => result.correct).length;
  const totalMastered = STATISTICS_FORMULAS.filter((card) => progress[card.id] === "mastered").length;
  ...
  function changeCardTopics(nextTopics: StatisticsTopicId[]) {
    ...
    setCardDeck(STATISTICS_FORMULAS.filter((card) => nextTopics.includes(card.topic)));
    setCardIndex(0);
  ...
  function changePracticeTopics(nextTopics: StatisticsTopicId[]) {
    ...
    setPracticeDeck(randomize(STATISTICS_QUESTIONS.filter((question) => nextTopics.includes(question.topic))));
    setPracticeIndex(0);
  ...
  function shufflePractice() {
    setPracticeDeck(randomize(STATISTICS_QUESTIONS.filter((question) => practiceTopics.includes(question.topic))));
    setPracticeIndex(0);
    setPracticeTypedAnswer("");
    setPracticeSelectedChoice("");
    setPracticeFeedback(null);
  ...
  function resumeSavedTest() {
    ...
    const results = savedTestSession.results.flatMap((savedResult) => {
      const question = STATISTICS_QUESTIONS.find((item) => item.id === savedResult.questionId);
  ...
  const pageStyle = { "--subject-accent": "#66e39e", "--english-yellow": "#66e39e" } as CSSProperties;
    ...
    <div className="app-frame generic-subject-page english-page statistics-page" style={pageStyle}>
      <header className="topbar">
        <Link className="brand" href="/" aria-label="9教科の一覧へ戻る">
          <span className="brand-mark hub-brand-mark" aria-hidden="true">P/S</span>
      ...
      <main className="generic-subject-main english-main statistics-main">
        <nav className="subject-breadcrumb statistics-breadcrumb" aria-label="現在位置">
          <Link href="/">科目一覧</Link><span>/</span><strong aria-current="page">確率統計</strong>
        </nav>
        ...
        <section className="generic-subject-hero english-hero statistics-hero" aria-labelledby="statistics-title">
          ...
          <button className="english-hero-memory-button statistics-hero-card-button" type="button" onClick={() => changeMode("cards")}>
            ...
            <small>{STATISTICS_FORMULAS.length}枚を単元別に暗記 →</small>
          </button>
        </section>
        ...
        <section ref={workspaceRef} id="statistics-workspace" className="english-workspace statistics-workspace">
          <div className="workspace-tabs english-tabs statistics-tabs" role="tablist" aria-label="確率統計の学習モード">
            ...
            <section className="english-guide-workspace statistics-scope-workspace" aria-labelledby="statistics-scope-title">
              ...
              <div className="english-guide-grid statistics-topic-grid">
                {STATISTICS_TOPICS.map((topic) => {
                  ...
                  const questionCount = STATISTICS_QUESTIONS.filter((question) => question.topic === topic.id).length;
                  return (
          ...
          {mode === "guide" && (
            <section className="english-guide-workspace statistics-guide-workspace" aria-labelledby="statistics-guide-title">
              ...
              <div className="english-guide-tip statistics-format-notice"><span>FORMAT ONLY</span><p>確率統計テスト①.pdf・確率統計テスト②.jpgの固有の問題、数値、解答は模擬テストへ入れていません。</p></div>
              <div className="english-guide-grid statistics-guide-grid">
                {STATISTICS_EXAM_FORMATS.map((format, index) => (
              ...
              <div className="english-guide-tip statistics-exam-tip"><span>EXAM ROUTINE</span><p><b>公式を書く → 数値を代入 → 丸める前の途中値を残す → 指定桁で答える。</b>連続小問では、前問の丸め前の値を次へ使います。</p></div>
            </section>
          )}
        </section>
