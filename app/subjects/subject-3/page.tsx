...
type TestResult = { question: MechanicalDynamicsQuestion; response: string; correct: boolean };
type SourceFilter = "all" | "range-zip" | "actual-exam";
type SavedTestSession = {
  version: 1;
  questionIds: string[];
  selectedTopics: MechanicalDynamicsTopicId[];
...
const PROGRESS_KEY = "test-grid:subject-3:progress:v1";
...
function parseNumericResponse(question: MechanicalDynamicsQuestion, value: string) {
...
function isCorrectAnswer(question: MechanicalDynamicsQuestion, response: string) {
...
function topicLabel(topicId: MechanicalDynamicsTopicId) {
  return MECHANICAL_DYNAMICS_TOPICS.find((topic) => topic.id === topicId)?.shortTitle ?? topicId;
}

function formatLabel(question: MechanicalDynamicsQuestion) {
...
function restoreTestSession(): SavedTestSession | null {
  ...
  try {
    ...
    const selectedTopics = Array.isArray(parsed.selectedTopics)
      ? parsed.selectedTopics.filter((topic): topic is MechanicalDynamicsTopicId => typeof topic === "string" && KNOWN_TOPIC_IDS.has(topic as MechanicalDynamicsTopicId))
      : [];
    ...
    return {
      ...
      sourceFilter: parsed.sourceFilter === "range-zip" || parsed.sourceFilter === "actual-exam" ? parsed.sourceFilter : "all",
...
function matchesSource(question: MechanicalDynamicsQuestion, source: SourceFilter) {
...
function TopicFilter({
  ...
  legend: string;
  selected: MechanicalDynamicsTopicId[];
  onChange: (topics: MechanicalDynamicsTopicId[]) => void;
}) {
  function toggle(topicId: MechanicalDynamicsTopicId) {
...
function AnswerForm({
  ...
  idPrefix: string;
  question: MechanicalDynamicsQuestion;
  typedAnswer: string;
  selectedChoice: string;
  feedback: Feedback | null;
...
function SolutionFeedback({
  ...
  return (
    <div className={`generic-test-answer english-test-feedback statistics-feedback ${feedback.correct ? "is-correct" : "is-wrong"}`} aria-live="polite">
      ...
      {question.diagram && <MechanicalDynamicsDiagram kind={question.diagram} solution title="模範図・注記" />}
...
export default function MechanicalDynamicsSubjectPage() {
  ...
  const [cardTopics, setCardTopics] = useState<MechanicalDynamicsTopicId[]>([...ALL_TOPIC_IDS]);
  const [cardDeck, setCardDeck] = useState([...MECHANICAL_DYNAMICS_FORMULAS]);
  ...
  const [testTopics, setTestTopics] = useState<MechanicalDynamicsTopicId[]>([...ALL_TOPIC_IDS]);
  ...
  const [testQuestions, setTestQuestions] = useState<MechanicalDynamicsQuestion[]>([]);
  ...
  const filteredCards = useMemo(
    () => MECHANICAL_DYNAMICS_FORMULAS.filter((card) => cardTopics.includes(card.topic)),
    [cardTopics],
  );
  ...
  const totalMastered = MECHANICAL_DYNAMICS_FORMULAS.filter((card) => progress[card.id] === "mastered").length;
  ...
  function jumpToFormulaCard(cardId: string) {
    const card = MECHANICAL_DYNAMICS_FORMULAS.find((item) => item.id === cardId);
    if (!card) return;
    const alreadyVisible = cardTopics.includes(card.topic);
    const nextTopics = alreadyVisible ? cardTopics : [...cardTopics, card.topic];
    const nextDeck = MECHANICAL_DYNAMICS_FORMULAS.filter((item) => nextTopics.includes(item.topic));
  ...
  function changeCardTopics(nextTopics: MechanicalDynamicsTopicId[]) {
    setCardTopics(nextTopics);
    setCardDeck(MECHANICAL_DYNAMICS_FORMULAS.filter((card) => nextTopics.includes(card.topic)));
    setCardIndex(0);
    setCardFlipped(false);
  }
  ...
  const pageStyle = { "--subject-accent": "#ff5c35", "--english-yellow": "#ff5c35" } as CSSProperties;
    ...
    <div className="app-frame generic-subject-page english-page statistics-page" style={pageStyle}>
      <header className="topbar">
        <Link className="brand" href="/" aria-label="9教科の一覧へ戻る">
          <span className="brand-mark hub-brand-mark" aria-hidden="true">M/D</span>
        ...
        <div className="header-actions statistics-header-actions">
          ...
          <Link className="outline-button header-link" href="/ranking/subject-3">公式ランキングテスト</Link>
          <Link className="outline-button header-link generated-practice-subject-link" href="/generated-practice?subject=subject-3">自動生成問題・共有履歴</Link>
      ...
      <main className="generic-subject-main english-main statistics-main">
        <nav className="subject-breadcrumb statistics-breadcrumb" aria-label="現在位置">
          <Link href="/">科目一覧</Link><span>/</span><strong aria-current="page">機械力学</strong>
        </nav>
        ...
        <section className="generic-subject-hero english-hero statistics-hero" aria-labelledby="statistics-title">
          <div className="english-hero-copy statistics-hero-copy">
            <p><span>SUBJECT 03</span><span>COURSE-RANGE ONLY</span></p>
            <h1 id="statistics-title">機械力学</h1>
            <small>範囲ZIPの{MECHANICAL_DYNAMICS_RANGE_PAGES.length}枚と機械力学過去問の全内容を学習し、公式・単位・線図・途中式を反復します。過去問は7大問・100点で、時間だけ資料に記載がないため模試では自分で設定できます。</small>
          </div>
          <button className="english-hero-memory-button statistics-hero-card-button" type="button" onClick={() => changeMode("cards")}>
            ...
            <small>{MECHANICAL_DYNAMICS_FORMULAS.length}枚を単元別に暗記 →</small>
          </button>
        </section>
        ...
        <section ref={workspaceRef} id="statistics-workspace" className="english-workspace statistics-workspace">
          <div className="workspace-tabs english-tabs statistics-tabs" role="tablist" aria-label="機械力学の学習モード">
            ...
            <button type="button" role="tab" aria-selected={mode === "expected"} className={mode === "expected" ? "active" : ""} onClick={() => changeMode("expected")}>⑤ 過去問・想定試験</button>
          ...
          {mode === "scope" && (
            <section className="english-guide-workspace statistics-scope-workspace" aria-labelledby="statistics-scope-title">
              <div className="english-panel-heading statistics-panel-heading">
                ...
                <p>テスト範囲ZIPの{MECHANICAL_DYNAMICS_RANGE_PAGES.length}枚と、機械力学過去問PDF全体から整理した{MECHANICAL_DYNAMICS_TOPICS.length}単元です。</p>
              </div>
              <div className="english-guide-tip statistics-source-policy">
                <span>SOURCE POLICY</span>
                <p><b>出題する：</b>範囲ZIP15枚と機械力学過去問PDFの全内容　／　<b>形式：</b>過去問の7大問・100点・途中式必須・単位必須を踏襲<br /><small>{MECHANICAL_DYNAMICS_SOURCE_POLICY.note} 試験時間だけ資料に記載がないため、模試では自由設定にします。</small></p>
              </div>
          ...
          {mode === "cards" && (
            <section className="generic-card-workspace english-card-workspace statistics-card-workspace" aria-labelledby="statistics-card-title">
              ...
              <CardDeckSearch
                items={MECHANICAL_DYNAMICS_FORMULAS.map((card) => ({
                  id: card.id,
                  label: card.title,
              ...
              {currentCard ? (
                <>
                  ...
                  <button type="button" className={`generic-flip-card english-flip-card statistics-flip-card mechanical-dynamics-flip-card ${cardFlipped ? "is-flipped" : ""}`} onClick={() => setCardFlipped((flipped) => !flipped)} aria-label={cardFlipped ? "問題面に戻る" : "公式を見る"}>
                  ...
                  {currentCard.diagram && <MechanicalDynamicsDiagram kind={currentCard.diagram} solution={cardFlipped} title={cardFlipped ? "公式と対応する模式図" : "問題の模式図"} />}
                  {cardFlipped && (
            ...
            <section className="generic-test-workspace english-test-workspace statistics-practice-workspace" aria-labelledby="statistics-practice-title">
              ...
              {currentPracticeQuestion ? (
                <div className="english-test-active statistics-practice-active">
                  ...
                  {currentPracticeQuestion.printedFormula && <div className="english-guide-tip statistics-question-context"><span>試験用紙に記載される式</span><DisplayMath tex={currentPracticeQuestion.printedFormula} /></div>}
                  {currentPracticeQuestion.diagram && <MechanicalDynamicsDiagram kind={currentPracticeQuestion.diagram} title="解答用線図" />}
            ...
            <section className="generic-test-workspace english-test-workspace statistics-test-workspace" aria-labelledby="statistics-test-title">
              {testPhase === "setup" && (
                <div className="english-test-setup statistics-test-setup">
                  <div className="english-panel-heading statistics-panel-heading"><div><span>MOCK EXAM</span><h2 id="statistics-test-title">模擬テストを作る</h2></div><p>選んだ単元と出典から、範囲ZIP・過去問由来の問題をランダム出題します。</p></div>
                  {savedTestSession && (
              ...
              {testPhase === "active" && currentTestQuestion && (
                <div className="english-test-active statistics-test-active">
                  ...
                  {currentTestQuestion.printedFormula && <div className="english-guide-tip statistics-question-context"><span>試験用紙に記載される式</span><DisplayMath tex={currentTestQuestion.printedFormula} /></div>}
                  {currentTestQuestion.diagram && <MechanicalDynamicsDiagram kind={currentTestQuestion.diagram} title="解答用線図" />}
                    ...
                    {testQuestions.map((question, index) => {
                      ...
                      return (
                        <article key={`${question.id}-${index}`} className={correct ? "is-correct" : "is-wrong"}>
                          ...
                          {question.diagram && <MechanicalDynamicsDiagram kind={question.diagram} solution title="振り返り用の模範図" />}
          ...
          {mode === "expected" && <MechanicalDynamicsExpectedExams />}

          {mode === "guide" && (
            <section className="english-guide-workspace statistics-guide-workspace" aria-labelledby="statistics-guide-title">
              <div className="english-panel-heading statistics-panel-heading"><div><span>FORMAT GUIDE</span><h2 id="statistics-guide-title">出題形式ガイド</h2></div><p>過去問は出題形式だけでなく、全問題を今回の試験範囲としてカード・演習・模試へ反映しています。</p></div>
              <div className="english-guide-tip statistics-format-notice"><span>SOURCE POLICY</span><p>範囲ZIPと過去問PDFはどちらも今回の範囲です。過去問の{MECHANICAL_DYNAMICS_EXAM_SPEC.bigQuestionCount}大問・{MECHANICAL_DYNAMICS_EXAM_SPEC.totalPoints}点・配点は確認済みで、試験時間だけ記載がありません。サイトでは時間を自由設定し、{MECHANICAL_DYNAMICS_EXAM_SPEC.passPoints}点を合格ラインとして練習します。</p></div>
              <div className="english-guide-grid statistics-guide-grid">
                {MECHANICAL_DYNAMICS_EXAM_FORMATS.map((format, index) => (
              ...
              <div className="english-guide-tip statistics-exam-tip"><span>EXAM ROUTINE</span><p><b>運動方向と座標を決める → 自由物体図を描く → 運動方程式を立てる → 数値代入 → 単位と有効数字3桁を整える。</b>過去問の指示どおり途中式を残し、π=3.14、g=9.80 m/s²を用います。</p></div>
            </section>
          )}
        </section>
      ...
      <footer className="english-footer statistics-footer"><span>TEST//GRID</span><p>MECHANICAL DYNAMICS · USER-SET TIME · PRACTICE SCORE /100 · LINE 60</p><span>SUBJECT 03</span></footer>
    </div>
  );
}
