...
type TestResult = { question: AppliedMathQuestion; response: string; correct: boolean };
type SavedTestSession = {
  version: 1;
  questionIds: string[];
  selectedTopics: AppliedMathTopicId[];
...
const PROGRESS_KEY = "test-grid:subject-8:progress:v1";
...
const KNOWN_QUESTION_IDS = new Set(APPLIED_MATH_QUESTIONS.map((question) => question.id));
...
function parseNumericResponse(value: string) {
  ...
  normalized = normalized.replace(/(?:通り|個|人|点|回|本|枚)$/u, "");
...
function isCorrectAnswer(question: AppliedMathQuestion, response: string) {
...
function topicLabel(topicId: AppliedMathTopicId) {
  return APPLIED_MATH_TOPICS.find((topic) => topic.id === topicId)?.shortTitle ?? topicId;
}

function formatLabel(question: AppliedMathQuestion) {
...
function restoreTestSession(): SavedTestSession | null {
  ...
  try {
    ...
    const selectedTopics = Array.isArray(parsed.selectedTopics)
      ? parsed.selectedTopics.filter((topic): topic is AppliedMathTopicId => typeof topic === "string" && KNOWN_TOPIC_IDS.has(topic as AppliedMathTopicId))
      : [];
...
function TopicFilter({
  ...
  legend: string;
  selected: AppliedMathTopicId[];
  onChange: (topics: AppliedMathTopicId[]) => void;
}) {
  function toggle(topicId: AppliedMathTopicId) {
...
export default function AppliedMathSubjectPage() {
  ...
  const [cardTopics, setCardTopics] = useState<AppliedMathTopicId[]>([...ALL_TOPIC_IDS]);
  const [cardDeck, setCardDeck] = useState([...APPLIED_MATH_FORMULAS]);
  const [cardIndex, setCardIndex] = useState(0);
  const [cardFlipped, setCardFlipped] = useState(false);

  const [practiceTopics, setPracticeTopics] = useState<AppliedMathTopicId[]>([...ALL_TOPIC_IDS]);
  const [practiceDeck, setPracticeDeck] = useState([...APPLIED_MATH_QUESTIONS]);
  ...
  const [testTopics, setTestTopics] = useState<AppliedMathTopicId[]>([...ALL_TOPIC_IDS]);
  const [questionCountDraft, setQuestionCountDraft] = useState("20");
  const [testQuestions, setTestQuestions] = useState<AppliedMathQuestion[]>([]);
  ...
  const filteredCards = useMemo(
    () => APPLIED_MATH_FORMULAS.filter((card) => cardTopics.includes(card.topic)),
    [cardTopics],
  );
  ...
  const availableTestQuestions = useMemo(
    () => APPLIED_MATH_QUESTIONS.filter((question) => testTopics.includes(question.topic)),
    [testTopics],
  );
  const currentTestQuestion = testQuestions[testIndex];
  const testScore = testResults.filter((result) => result.correct).length;
  const totalMastered = APPLIED_MATH_FORMULAS.filter((card) => progress[card.id] === "mastered").length;
  ...
  function jumpToFormulaCard(cardId: string) {
    const card = APPLIED_MATH_FORMULAS.find((item) => item.id === cardId);
    if (!card) return;
    const alreadyVisible = cardTopics.includes(card.topic);
    const nextTopics = alreadyVisible ? cardTopics : [...cardTopics, card.topic];
    const nextDeck = APPLIED_MATH_FORMULAS.filter((item) => nextTopics.includes(item.topic));
  ...
  function changeCardTopics(nextTopics: AppliedMathTopicId[]) {
    setCardTopics(nextTopics);
    setCardDeck(APPLIED_MATH_FORMULAS.filter((card) => nextTopics.includes(card.topic)));
    setCardIndex(0);
    setCardFlipped(false);
  }
  ...
  function changePracticeTopics(nextTopics: AppliedMathTopicId[]) {
    ...
    setPracticeDeck(randomize(APPLIED_MATH_QUESTIONS.filter((question) => nextTopics.includes(question.topic))));
    setPracticeIndex(0);
  ...
  function shufflePractice() {
    setPracticeDeck(randomize(APPLIED_MATH_QUESTIONS.filter((question) => practiceTopics.includes(question.topic))));
    setPracticeIndex(0);
    setPracticeTypedAnswer("");
    setPracticeSelectedChoice("");
    setPracticeFeedback(null);
  ...
  function acceptPracticeText() {
    if (!practiceFeedback || !currentPracticeQuestion || currentPracticeQuestion.format !== "text") return;
  ...
  function acceptTestText() {
    if (!testFeedback || !currentTestQuestion || currentTestQuestion.format !== "text") return;
  ...
  function resumeSavedTest() {
    ...
    const questions = savedTestSession.questionIds.flatMap((id) => {
      const found = APPLIED_MATH_QUESTIONS.find((question) => question.id === id);
      return found ? [found] : [];
    });
    if (!questions.length) {
      deleteSavedTest();
    ...
    const results = savedTestSession.results.flatMap((savedResult) => {
      const question = APPLIED_MATH_QUESTIONS.find((item) => item.id === savedResult.questionId);
  ...
  const pageStyle = { "--subject-accent": "#7aa7ff", "--english-yellow": "#7aa7ff" } as CSSProperties;
    ...
    <div className="app-frame generic-subject-page english-page statistics-page" style={pageStyle}>
      <header className="topbar">
        <Link className="brand" href="/" aria-label="9教科の一覧へ戻る">
          <span className="brand-mark hub-brand-mark" aria-hidden="true">A/M</span>
      ...
      <main className="generic-subject-main english-main statistics-main">
        <nav className="subject-breadcrumb statistics-breadcrumb" aria-label="現在位置">
          <Link href="/">科目一覧</Link><span>/</span><strong aria-current="page">応用数学</strong>
        </nav>
        ...
        <section className="generic-subject-hero english-hero statistics-hero" aria-labelledby="statistics-title">
          ...
          <button className="english-hero-memory-button statistics-hero-card-button" type="button" onClick={() => changeMode("cards")}>
            ...
            <small>{APPLIED_MATH_FORMULAS.length}枚を単元別に暗記 →</small>
          </button>
        </section>
        ...
        <section ref={workspaceRef} id="statistics-workspace" className="english-workspace statistics-workspace">
          <div className="workspace-tabs english-tabs statistics-tabs" role="tablist" aria-label="応用数学の学習モード">
            ...
            <section className="english-guide-workspace statistics-scope-workspace" aria-labelledby="statistics-scope-title">
              ...
              <div className="english-guide-grid statistics-topic-grid">
                {APPLIED_MATH_TOPICS.map((topic) => {
                  ...
                  const questionCount = APPLIED_MATH_QUESTIONS.filter((question) => question.topic === topic.id).length;
                  return (
          ...
          {mode === "cards" && (
            <section className="generic-card-workspace english-card-workspace statistics-card-workspace" aria-labelledby="statistics-card-title">
              ...
              <CardDeckSearch
                items={APPLIED_MATH_FORMULAS.map((card) => ({
                  id: card.id,
                  label: card.title,
            ...
            <section className="generic-test-workspace english-test-workspace statistics-test-workspace" aria-labelledby="statistics-test-title">
              {testPhase === "setup" && (
                <div className="english-test-setup statistics-test-setup">
                  <div className="english-panel-heading statistics-panel-heading"><div><span>MOCK EXAM</span><h2 id="statistics-test-title">模擬テストを作る</h2></div><p>選んだ単元の範囲問題だけをランダム出題します。見本テストの問題文・数値は含みません。</p></div>
                  {savedTestSession && (
              ...
              {testPhase === "result" && (
                <div className="english-test-result statistics-test-result">
                  <span>MOCK EXAM RESULT</span><h2>{testScore} / {testResults.length}</h2><p>正答率 {testResults.length ? Math.round((testScore / testResults.length) * 100) : 0}%・経過 {formatElapsedTime(testElapsedSeconds)}</p>
          ...
          {mode === "guide" && (
            <section className="english-guide-workspace statistics-guide-workspace" aria-labelledby="statistics-guide-title">
              <div className="english-panel-heading statistics-panel-heading"><div><span>FORMAT GUIDE</span><h2 id="statistics-guide-title">出題形式ガイド</h2></div><p>過去のテストは範囲としてではなく、解答の組み立て方と時間配分を知る見本として反映しています。</p></div>
              <div className="english-guide-tip statistics-format-notice"><span>FORMAT ONLY</span><p>形式1.pdf・形式2.pdf・形式3.zipの固有の問題、数値、範囲外単元は模擬テストへ入れていません。確認できた50分・80点・大問6問・途中式欄だけを反映します。</p></div>
              <div className="english-guide-grid statistics-guide-grid">
                {APPLIED_MATH_EXAM_FORMATS.map((format, index) => (
              ...
              <div className="english-guide-tip statistics-exam-tip"><span>EXAM ROUTINE</span><p><b>公式を書く → ベクトルや偏微分を成分で展開 → 符号と正規化を確認 → 最終式を整える。</b>外積・ノルム・積分の途中式を残すと部分点を狙えます。</p></div>
            </section>
          )}
        </section>
      ...
      <footer className="english-footer statistics-footer"><span>TEST//GRID</span><p>APPLIED MATHEMATICS · 50 MIN · 80 PTS · PASS 48</p><span>SUBJECT 08</span></footer>
    </div>
  );
}
