...
import { THERMODYNAMICS_EXPECTED_EXAMS } from "./thermodynamics-data";
import ThermodynamicsDiagram, { type ThermodynamicsDiagramKind } from "./thermodynamics-diagrams";
import { DisplayMath, RichMathText } from "./statistics-math";
import styles from "./thermodynamics-expected-exams.module.css";
...
export const THERMODYNAMICS_EXPECTED_EXAM_STORAGE_KEY = "test-grid:subject-4:expected-exam:v1";
const DEFAULT_PRACTICE_MINUTES = 50;
const PASS_PERCENT = 60;
const SECTIONS_PER_PAGE = 2;
const EXAMS = THERMODYNAMICS_EXPECTED_EXAMS as unknown as ExpectedExam[];
...
function normalizeUnitToken(value: string) {
  return value.normalize("NFKC").toLocaleLowerCase("en").replace(/\s+/g, "").replace(/℃/g, "°c").replace(/\^3/g, "³");
}

function parseLooseNumber(question: ExpectedQuestion, response: string) {
  let normalized = response.normalize("NFKC").toLocaleLowerCase("en").replace(/[−–—]/g, "-").replace(/,/g, "").replace(/\s+/g, "").trim();
...
function sourceLabel(question: ExpectedQuestion) {
  ...
  const includesOverlap = question.sourceRefs?.some((source) => source.kind === "format3-overlap");
  if (pages.length) return `${includesOverlap ? "範囲資料・形式3確認済み重複" : "範囲資料"} p.${pages.join("・")}`;
  return includesOverlap ? "形式3の確認済み範囲重複" : "試験範囲資料";
}

function parseStoredExam(raw: string | null): StoredThermodynamicsExam | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<StoredThermodynamicsExam>;
...
function ThermodynamicsExpectedExamLab({ firstExam }: { firstExam: ExpectedExam }) {
  ...
  const [savedExam, setSavedExam] = useState<StoredThermodynamicsExam | null>(null);
  ...
  useEffect(() => {
    setSavedExam(parseStoredExam(window.localStorage.getItem(THERMODYNAMICS_EXPECTED_EXAM_STORAGE_KEY)));
    setHydrated(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const snapshot: StoredThermodynamicsExam = {
        version: 1,
        examId: exam.id,
      ...
      try { window.localStorage.setItem(THERMODYNAMICS_EXPECTED_EXAM_STORAGE_KEY, JSON.stringify(snapshot)); } catch { /* Continue without storage. */ }
    }, 250);
    return () => window.clearTimeout(timer);
  }, [answers, awardedPoints, exam.id, hydrated, phase, practiceMinutes, remainingSeconds]);

  useEffect(() => () => document.body.classList.remove("thermodynamics-expected-printing"), []);
  ...
  function startExam(examId: string) {
    ...
    window.setTimeout(() => document.getElementById("thermodynamics-expected-paper")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  }
  ...
  function deleteSavedExam() {
    try { window.localStorage.removeItem(THERMODYNAMICS_EXPECTED_EXAM_STORAGE_KEY); } catch { /* Ignore unavailable storage. */ }
    setSavedExam(null);
  ...
  function printExam(includeAnswers: boolean) {
    ...
    const cleanup = () => document.body.classList.remove("thermodynamics-expected-printing");
    document.body.classList.add("thermodynamics-expected-printing");
  ...
  if (phase === "setup") {
    return (
      <section className={styles.selector} id="thermodynamics-expected-selector" aria-labelledby="thermodynamics-expected-title">
        <header className={styles.selectorHeader}>
          <div><span>EXPECTED EXAM · PRACTICE SETTINGS</span><h2 id="thermodynamics-expected-title">熱力学 想定試験</h2></div>
          <p>正式な試験時間・満点・配点は資料から確認できません。時間は自分で設定し、得点は練習用100点換算・60点ラインで確認します。</p>
        </header>
        ...
        <div className={styles.examGrid}>
          {EXAMS.map((candidate) => (
            <article className={styles.examCard} key={candidate.id}>
              ...
              <h3>{candidate.title}</h3><p>{candidate.subtitle ?? "熱力学・全範囲練習"}</p>
  ...
  return (
    <section className={`${styles.lab} ${printAnswers ? styles.printAnswers : styles.printQuestions}`} aria-labelledby="thermodynamics-active-title">
      ...
      <div className={styles.paperStack} id="thermodynamics-expected-paper">
        {pages.map((pageSections, pageIndex) => (
          <article className={styles.paperPage} key={`${exam.id}-page-${pageIndex + 1}`}>
            <header className={styles.paperHeader}><div><span>THERMODYNAMICS · PRACTICE PAPER</span><h1 id={pageIndex === 0 ? "thermodynamics-active-title" : undefined}>{exam.title}</h1><p>{exam.subtitle ?? "範囲資料準拠・練習用想定問題"}</p></div><label><span>学生番号・氏名</span><input type="text" aria-label={`学生番号・氏名・${pageIndex + 1}ページ`} /></label></header>
            ...
            {pageSections.map((section) => (
              <section className={styles.examSection} key={`${exam.id}-section-${section.number}`}>
                <header className={styles.sectionHeader}><span>第{section.number}問</span><div><h2>{section.title}</h2><small>{section.topicIds?.join(" / ") ?? "THERMODYNAMICS"}</small></div><strong>{section.points}点</strong></header>
                ...
                {section.questions.map((question) => (
                  <article className={styles.paperQuestion} key={question.id}>
                    ...
                    {question.diagram && <ThermodynamicsDiagram className={styles.diagram} kind={question.diagram} title={`${question.genre}：解答用線図`} />}
      ...
      {phase === "result" && (
        <section className={styles.answerBook} aria-labelledby="thermodynamics-solutions-title">
          <header className={styles.resultHeader}><div><span>ANSWER · UNITS · STEPS · SOURCE</span><h2 id="thermodynamics-solutions-title">模範解答と途中式</h2><p>完全一致・数値・キーワードは仮採点済みです。同値な式、単位、有効数字、線図の部分点を各小問で調整してください。</p></div><strong>{convertedScore >= PASS_PERCENT ? "PASS" : "RETRY"}</strong></header>
                ...
                {section.questions.map((question) => {
                  ...
                  return (
                    <article className={styles.solutionCard} key={`solution-${question.id}`}>
                      ...
                      {question.diagram && <ThermodynamicsDiagram className={styles.diagram} kind={question.diagram} solution title={`${question.genre}：模範線図`} />}
