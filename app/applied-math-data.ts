...
const APPLIED_MATH_FORMULA_DEFINITIONS: Array<Omit<AppliedMathFormulaCard, "sourcePages">> = [
  ...
  { id: "am-source-surface-unit-normal", topic: "surfaces", title: "追加演習の単位法線", prompt: "\\(\\mathbf r=(u+v,u-v,(u^2+v^2)/2)\\) の単位法線は？", formula: "\\mathbf n=\\pm\\frac{(u+v,u-v,-2)}{\\sqrt{2u^2+2v^2+4}}", explanation: "\\(\\mathbf r_u=(1,1,u)\\)、\\(\\mathbf r_v=(1,-1,v)\\) の外積を作り、その大きさで割る。法線方向の指定がなければ両向きを認める。", cue: "外積は (u+v,u-v,-2)", example: "\\((u,v)=(0,0)\\) では \\(\\mathbf n=\\pm(0,0,-1)\\)" },
  ...
  { id: "am-div-curl-linearity", topic: "divergence-curl", title: "発散・回転の線形性（補助）", prompt: "ベクトル場の和の発散と回転は？", formula: "\\nabla\\cdot(\\mathbf a+\\mathbf b)=\\nabla\\cdot\\mathbf a+\\nabla\\cdot\\mathbf b,\\qquad\\nabla\\times(\\mathbf a+\\mathbf b)=\\nabla\\times\\mathbf a+\\nabla\\times\\mathbf b", explanation: "偏微分の線形性により、和を先に計算しても各場へ分けてから計算しても同じになる補助公式。", cue: "divもcurlも和へ分配" },
  { id: "am-div-curl-scalar-product", topic: "divergence-curl", title: "スカラー倍の発散・回転（補助）", prompt: "スカラー場 \\(\\phi\\) とベクトル場 \\(\\mathbf a\\) の積にdiv・curlを作用させると？", formula: "\\nabla\\cdot(\\phi\\mathbf a)=\\nabla\\phi\\cdot\\mathbf a+\\phi\\nabla\\cdot\\mathbf a,\\qquad\\nabla\\times(\\phi\\mathbf a)=\\nabla\\phi\\times\\mathbf a+\\phi\\nabla\\times\\mathbf a", explanation: "積の微分と同様に、スカラー場を微分する項とベクトル場を微分する項へ分かれる。curlでは外積の順序を変えない。", cue: "∇φの項＋φを残す項" },
  ...
  { id: "am-gradient-quotient", topic: "gradient", title: "勾配の逆数・商", prompt: "逆数と商の勾配公式は？", formula: "\\nabla\\!\\left(\\frac1\\psi\\right)=-\\frac{\\nabla\\psi}{\\psi^2},\\qquad\\nabla\\!\\left(\\frac\\phi\\psi\\right)=\\frac{\\psi\\nabla\\phi-\\phi\\nabla\\psi}{\\psi^2}", explanation: "一変数の逆数・商の微分を、各座標方向の偏微分へ同時に適用した形。分母が0でない範囲で使う。", cue: "商は分母×分子勾配−分子×分母勾配" },
  ...
  { id: "am-vector-line-integral", topic: "line-integrals", title: "ベクトル場の線積分", prompt: "ベクトル場 \\(\\mathbf a\\) の線積分は？", formula: "\\int_C\\mathbf a\\cdot d\\mathbf r=\\int_a^b\\mathbf a(\\mathbf r(t))\\cdot\\mathbf r'(t)\\,dt=\\int_C(a_x\\,dx+a_y\\,dy+a_z\\,dz)", explanation: "ベクトル場の接線方向成分を曲線に沿って積分する。力の場なら経路に沿って行う仕事に対応する。", cue: "場と接ベクトルの内積" },
  { id: "am-line-path-rules", topic: "line-integrals", title: "線積分の経路法則", prompt: "経路の結合・反転でベクトル線積分はどう変わる？", formula: "\\int_{C_1+C_2}\\mathbf a\\cdot d\\mathbf r=\\int_{C_1}\\mathbf a\\cdot d\\mathbf r+\\int_{C_2}\\mathbf a\\cdot d\\mathbf r,\\qquad\\int_{-C}\\mathbf a\\cdot d\\mathbf r=-\\int_C\\mathbf a\\cdot d\\mathbf r", explanation: "経路をつなげれば各区間の積分を足す。逆向きでは位置の微小変化が反転するため符号も反転する。", cue: "つなぐと足す、逆向きはマイナス" },
...
export const APPLIED_MATH_QUESTIONS: AppliedMathQuestion[] = [
  ...
  courseQuestion({ id: "am-q-source-surface-unit-normal", topic: "surfaces", genre: "追加範囲・単位法線", difficulty: 2, format: "choice", prompt: "\\(\\mathbf r(u,v)=(u+v,u-v,(u^2+v^2)/2)\\) の単位法線を選べ。", answer: "\\(\\displaystyle\\pm\\frac{(u+v,u-v,-2)}{\\sqrt{2u^2+2v^2+4}}\\)", options: ["\\(\\displaystyle\\pm\\frac{(u+v,u-v,-2)}{\\sqrt{2u^2+2v^2+4}}\\)", "\\(\\displaystyle\\pm\\frac{(u-v,u+v,2)}{\\sqrt{2u^2+2v^2+4}}\\)", "\\((u+v,u-v,-2)\\)", "\\(\\displaystyle\\pm\\frac{(1,1,u)}{\\sqrt{u^2+2}}\\)"], formula: "\\mathbf n=\\pm(\\mathbf r_u\\times\\mathbf r_v)/|\\mathbf r_u\\times\\mathbf r_v|", steps: ["\\(\\mathbf r_u=(1,1,u),\\ \\mathbf r_v=(1,-1,v)\\)", "\\(\\mathbf r_u\\times\\mathbf r_v=(u+v,u-v,-2)\\)", "大きさ \\(\\sqrt{2u^2+2v^2+4}\\) で割る"], explanation: "外積の第2成分は \\(u-v\\)、第3成分は \\(-2\\)。向きの指定がないので全体を反転した法線も正しい。" }),
  ...
  courseQuestion({ id: "am-q-source-directional", topic: "gradient", genre: "追加範囲・方向微分", difficulty: 3, format: "number", prompt: "\\(\\phi=e^{3x}+y^2\\log z\\), \\(P=(0,2,1)\\) における \\(\\mathbf a=(2,1,-2)\\) 方向の方向微分を求めよ。", answer: "\\(-2/3\\)", numericAnswer: -2 / 3, tolerance: 0.0001, formula: "D_{\\mathbf e}\\phi=\\nabla\\phi(P)\\cdot\\frac{\\mathbf a}{|\\mathbf a|}", steps: ["\\(\\nabla\\phi=(3e^{3x},2y\\log z,y^2/z)\\) より \\(\\nabla\\phi(P)=(3,0,4)\\)", "\\(|\\mathbf a|=3\\) なので \\(\\mathbf e=(2,1,-2)/3\\)", "\\((3,0,4)\\cdot(2,1,-2)/3=-2/3\\)"], explanation: "方向ベクトルを単位化してから勾配との内積を取る。単位化を忘れると値が3倍になる。" }),
  courseQuestion({ id: "am-q-gradient-quotient-rule", topic: "gradient", genre: "勾配の演算法則", difficulty: 2, format: "choice", prompt: "\\(\\nabla(\\phi/\\psi)\\) の正しい公式を選べ。ただし \\(\\psi\\ne0\\) とする。", answer: "\\((\\psi\\nabla\\phi-\\phi\\nabla\\psi)/\\psi^2\\)", options: ["\\((\\psi\\nabla\\phi-\\phi\\nabla\\psi)/\\psi^2\\)", "\\((\\phi\\nabla\\psi-\\psi\\nabla\\phi)/\\psi^2\\)", "\\((\\nabla\\phi)/(\\nabla\\psi)\\)", "\\((\\nabla\\phi+\\nabla\\psi)/\\psi\\)"], steps: ["一変数の商の微分と同じ順序", "分母を二乗し、分子は分母×分子勾配−分子×分母勾配"], explanation: "勾配どうしを割るのではない。分子の引き算の順序と分母の二乗が重要。" }),
  ...
  courseQuestion({ id: "am-q-scalar-line-ds", topic: "line-integrals", genre: "スカラー線積分", difficulty: 2, format: "number", prompt: "\\(C:\\mathbf r(t)=(\\cos t,\\sin t,1)\\), \\(0\\le t\\le\\pi\\) のとき \\(\\int_C(x^2+z)\\,ds\\) を \\(\\pi\\) の係数で答えよ。", answer: "\\(3\\pi/2\\)（係数3/2）", numericAnswer: 3 / 2, tolerance: 0.0001, formula: "\\int_C\\phi\\,ds=\\int_a^b\\phi(\\mathbf r(t))|\\mathbf r'(t)|dt", steps: ["\\(|\\mathbf r'(t)|=1\\)", "\\(x^2+z=\\cos^2t+1\\)", "\\(\\int_0^\\pi(\\cos^2t+1)dt=3\\pi/2\\)"], explanation: "入力欄には \\(\\pi\\) の係数 \\(3/2\\) を入力する。dsでは速さを掛ける。" }),
  courseQuestion({ id: "am-q-scalar-line-dx", topic: "line-integrals", genre: "成分線積分", difficulty: 2, format: "number", prompt: "同じ曲線で \\(\\int_C(x^2+z)\\,dx\\) を求めよ。", context: "\\(\\mathbf r(t)=(\\cos t,\\sin t,1),\\ 0\\le t\\le\\pi\\)", answer: "\\(-8/3\\)", numericAnswer: -8 / 3, tolerance: 0.0001, formula: "\\int_C\\phi\\,dx=\\int_a^b\\phi(\\mathbf r(t))x'(t)dt", steps: ["\\(dx=x'(t)dt=-\\sin t\\,dt\\)", "\\(\\int_0^\\pi(\\cos^2t+1)(-\\sin t)dt\\)", "\\(u=\\cos t\\) とおけば \\(\\int_1^{-1}(u^2+1)du=-8/3\\)"], explanation: "dxでは曲線長ではなくx成分の変化を掛けるため、向きによって符号が変わる。" }),
  courseQuestion({ id: "am-q-scalar-line-helix-ds", topic: "line-integrals", genre: "スカラー線積分", difficulty: 3, format: "text", prompt: "\\(C:\\mathbf r(t)=(\\cos t,\\sin t,t)\\), \\(0\\le t\\le\\pi/2\\) のとき \\(\\int_C(x+y^2)\\,ds\\) を求めよ。", answer: "\\(\\sqrt2(1+\\pi/4)\\)", accepted: ["sqrt2(1+pi/4)", "√2(1+π/4)"], keywords: ["2", "1", "4"], minKeywords: 3, formula: "\\int_C\\phi\\,ds=\\int_a^b\\phi(\\mathbf r(t))|\\mathbf r'(t)|dt", steps: ["\\(|\\mathbf r'|=\\sqrt{\\sin^2t+\\cos^2t+1}=\\sqrt2\\)", "\\(x+y^2=\\cos t+\\sin^2t\\)", "\\(\\sqrt2\\int_0^{\\pi/2}(\\cos t+\\sin^2t)dt=\\sqrt2(1+\\pi/4)\\)"], explanation: "らせんではz成分の導関数も速さに含めるため、面内の円より速さが \\(\\sqrt2\\) になる。" }),
  courseQuestion({ id: "am-q-scalar-line-dy", topic: "line-integrals", genre: "成分線積分", difficulty: 3, format: "text", prompt: "同じ曲線で \\(\\int_C(x+y^2)\\,dy\\) を求めよ。", context: "\\(\\mathbf r(t)=(\\cos t,\\sin t,t),\\ 0\\le t\\le\\pi/2\\)", answer: "\\(\\pi/4+1/3\\)", accepted: ["pi/4+1/3", "π/4+1/3"], keywords: ["4", "1", "3"], minKeywords: 3, formula: "\\int_C\\phi\\,dy=\\int_a^b\\phi(\\mathbf r(t))y'(t)dt", steps: ["\\(dy=\\cos t\\,dt\\)", "\\(\\int_0^{\\pi/2}(\\cos t+\\sin^2t)\\cos t\\,dt\\)", "\\(\\int_0^{\\pi/2}\\cos^2t\\,dt+\\int_0^{\\pi/2}\\sin^2t\\cos t\\,dt=\\pi/4+1/3\\)"], explanation: "dyなのでyの導関数 \\(\\cos t\\) を掛ける。dsの \\(\\sqrt2\\) は使わない。" }),
  courseQuestion({ id: "am-q-vector-line-one", topic: "line-integrals", genre: "ベクトル線積分", difficulty: 3, format: "number", prompt: "\\(C:\\mathbf r(t)=(t^2+1,2t,1)\\), \\(0\\le t\\le1\\)、\\(\\mathbf a=(xy,yz,zx)\\) の \\(\\int_C\\mathbf a\\cdot d\\mathbf r\\) を求めよ。", answer: "\\(62/15\\)", numericAnswer: 62 / 15, tolerance: 0.0001, formula: "\\int_C\\mathbf a\\cdot d\\mathbf r=\\int_a^b\\mathbf a(\\mathbf r(t))\\cdot\\mathbf r'(t)dt", steps: ["\\(\\mathbf a(\\mathbf r(t))=(2t(t^2+1),2t,t^2+1)\\)", "\\(\\mathbf r'(t)=(2t,2,0)\\)", "内積は \\(4t^4+4t^2+4t\\)", "\\(\\int_0^1(4t^4+4t^2+4t)dt=62/15\\)"], explanation: "場へ曲線を代入してから接ベクトルとの内積を作り、最後にtで積分する。" }),
  ...
  courseQuestion({ id: "am-q-gradient-theorem", topic: "line-integrals", genre: "勾配定理", difficulty: 1, format: "choice", prompt: "\\(\\mathbf a=\\nabla\\phi\\) のとき、点AからBへ至る曲線C上の線積分として正しいものを選べ。", answer: "\\(\\phi(B)-\\phi(A)\\)", options: ["\\(\\phi(B)-\\phi(A)\\)", "\\(\\phi(A)-\\phi(B)\\)", "\\(\\phi(A)+\\phi(B)\\)", "経路ごとに必ず異なる"], steps: ["合成関数の微分で被積分関数は \\(d\\phi(\\mathbf r(t))/dt\\)", "端点で評価して \\(\\phi(B)-\\phi(A)\\)"], explanation: "勾配場の線積分は経路独立で、始点と終点のスカラー場の値だけで決まる。" }),
  ...
  courseQuestion({ id: "am-q-flux-source", topic: "surface-integrals", genre: "流束面積分", difficulty: 3, format: "number", prompt: "\\(\\mathbf r(u,v)=(u,v,\\sqrt{9-u^2})\\), \\(0\\le u\\le3,0\\le v\\le4\\)、\\(\\mathbf a=(6z,2x+y,-x)\\) とする。z成分が正の法線方向の流束 \\(\\int_S\\mathbf a\\cdot\\mathbf n\\,dS\\) を求めよ。", answer: "90", numericAnswer: 90, formula: "\\int_S\\mathbf a\\cdot\\mathbf n\\,dS=\\iint_D\\mathbf a(\\mathbf r)\\cdot(\\mathbf r_u\\times\\mathbf r_v)dudv", steps: ["\\(\\mathbf r_u\\times\\mathbf r_v=(u/\\sqrt{9-u^2},0,1)\\) はz成分が正", "\\(\\mathbf a(\\mathbf r)\\cdot(\\mathbf r_u\\times\\mathbf r_v)=6u-u=5u\\)", "\\(\\int_0^4\\int_0^3 5u\\,du\\,dv=90\\)"], explanation: "単位法線とdSを別々に計算する代わりに、向き付き外積をそのまま使うと簡潔になる。" }),

  courseQuestion({ id: "am-q-green-square", topic: "green-theorem", genre: "グリーンの定理", difficulty: 3, format: "number", prompt: "正方形 \\(0\\le x\\le1,0\\le y\\le1\\) の境界Cを反時計回りに一周するとき、\\(\\oint_C\\{(x^2+2y^2)dx+xy^2dy\\}\\) を求めよ。", answer: "\\(-5/3\\)", numericAnswer: -5 / 3, tolerance: 0.0001, formula: "\\oint_C(Pdx+Qdy)=\\iint_D(Q_x-P_y)dA", steps: ["\\(P=x^2+2y^2,\\ Q=xy^2\\)", "\\(Q_x-P_y=y^2-4y\\)", "\\(\\int_0^1\\int_0^1(y^2-4y)\\,dy\\,dx=-5/3\\)"], explanation: "O→A→B→Cの順は反時計回りの正向き。微分の順序は \\(Q_x-P_y\\) である。" }),
...
function buildAppliedMathExam(variant: number): AppliedMathExpectedExam {
  ...
  const sections: AppliedMathExamSection[] = [
    ...
    },
    {
      ...
      questions: [
        examQuestion(id, 4, 1, 4, { topic: "surfaces", genre: "単位法線", difficulty: 2, format: "text", prompt: "\\(\\mathbf r_u\\times\\mathbf r_v\\) と単位法線の1つを求めよ。", answer: `\\(\\mathbf r_u\\times\\mathbf r_v=(-${surfaceP},${surfaceQ},1),\\ \\mathbf n=\\frac{(-${surfaceP},${surfaceQ},1)}{\\sqrt{${surfaceNormSquared}}}\\)`, accepted: [`(-${surfaceP},${surfaceQ},1)/sqrt(${surfaceNormSquared})`], keywords: [`-${surfaceP}`, String(surfaceQ), String(surfaceNormSquared)], minKeywords: 3, formula: "\\mathbf n=\\pm(\\mathbf r_u\\times\\mathbf r_v)/|\\mathbf r_u\\times\\mathbf r_v|", steps: [`\\(\\mathbf r_u=(1,0,${surfaceP}),\\ \\mathbf r_v=(0,1,-${surfaceQ})\\)`, `\\(\\mathbf r_u\\times\\mathbf r_v=(-${surfaceP},${surfaceQ},1)\\)`, `大きさ \\(\\sqrt{${surfaceNormSquared}}\\) で割る`], explanation: "逆向きの単位法線も正しい。" }),
    ...
    },
    {
      ...
      questions: [
        ...
        examQuestion(id, 6, 3, 3, { topic: "surface-integrals", genre: "流束面積分", difficulty: 3, format: "number", prompt: `\\(\\mathbf r(u,v)=(u,v,\\sqrt{${fluxRadius ** 2}-u^2})\\), \\(0\\le u\\le${fluxRadius},0\\le v\\le${fluxHeight}\\)、\\(\\mathbf a=(${fluxFieldCoefficient}z,0,-x)\\) とする。z成分が正の法線方向の流束を求めよ。`, answer: String(fluxValue), numericAnswer: fluxValue, formula: "\\int_S\\mathbf a\\cdot\\mathbf n\\,dS=\\iint_D\\mathbf a(\\mathbf r)\\cdot(\\mathbf r_u\\times\\mathbf r_v)dudv", steps: [`\\(\\mathbf r_u\\times\\mathbf r_v=(u/\\sqrt{${fluxRadius ** 2}-u^2},0,1)\\)`, `内積は \\((${fluxFieldCoefficient}-1)u=${fluxFieldCoefficient - 1}u\\)`, `\\(\\int_0^{${fluxHeight}}\\int_0^{${fluxRadius}}${fluxFieldCoefficient - 1}u\\,du\\,dv=${fluxValue}\\)`], explanation: "法線方向を外積の向きで合わせ、単位法線と面積要素を向き付き外積へまとめる。" }),
