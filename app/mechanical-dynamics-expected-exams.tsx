...
import { MECHANICAL_DYNAMICS_ACTUAL_EXAM, MECHANICAL_DYNAMICS_EXPECTED_EXAMS } from "./mechanical-dynamics-data";
import MechanicalDynamicsDiagram, { type MechanicalDynamicsDiagramKind } from "./mechanical-dynamics-diagrams";
import { DisplayMath, RichMathText } from "./statistics-math";
import styles from "./mechanical-dynamics-expected-exams.module.css";

type SourceRef = { kind?: string; page?: number; major?: number; question?: number; sub?: number; note?: string; evidence?: string };
...
type StoredMechanicalDynamicsExam = {
  version: 1;
  examId: string;
  phase: "paused" | "result";
  answers: AnswerMap;
...
export const MECHANICAL_DYNAMICS_EXPECTED_EXAM_STORAGE_KEY = "test-grid:subject-3:expected-exam:v1";
const DEFAULT_PRACTICE_MINUTES = 50;
const PASS_PERCENT = 60;
const SECTIONS_PER_PAGE = 7;
const EXAMS = [MECHANICAL_DYNAMICS_ACTUAL_EXAM, ...MECHANICAL_DYNAMICS_EXPECTED_EXAMS] as unknown as ExpectedExam[];
...
function isActualExam(exam: ExpectedExam) {
  return exam.kind === "actual" || exam.id.includes("actual");
}

function sourceLabel(question: ExpectedQuestion) {
  const refs = question.sourceRefs ?? [];
  const labels: string[] = [];
  const rangePages = refs.filter((source) => source.kind === "range-zip" && typeof source.page === "number").map((source) => source.page as number);
  if (rangePages.length) labels.push(`範囲ZIP p.${Array.from(new Set(rangePages)).sort((a, b) => a - b).join("・")}`);
  for (const source of refs.filter((candidate) => candidate.kind === "actual-exam")) {
    const major = source.major ?? source.question;
    labels.push(`実物過去問${major ? ` 第${major}問` : ""}${source.sub ? `（${source.sub}）` : ""}`);
  }
  if (!labels.length && question.sourcePages?.length) labels.push(`範囲ZIP p.${question.sourcePages.join("・")}`);
  return labels.length ? Array.from(new Set(labels)).join("／") : "範囲ZIP・実物過去問に基づく問題";
}

function parseStoredExam(raw: string | null): StoredMechanicalDynamicsExam | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<StoredMechanicalDynamicsExam>;
...
function MechanicalDynamicsExpectedExamLab({ firstExam }: { firstExam: ExpectedExam }) {
  ...
  const [savedExam, setSavedExam] = useState<StoredMechanicalDynamicsExam | null>(null);
  ...
  const actualExam = isActualExam(exam);
  const paperPageCount = pages.length + 1;
  ...
  useEffect(() => {
    setSavedExam(parseStoredExam(window.localStorage.getItem(MECHANICAL_DYNAMICS_EXPECTED_EXAM_STORAGE_KEY)));
    setHydrated(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const snapshot: StoredMechanicalDynamicsExam = {
        version: 1,
        examId: exam.id,
      ...
      try { window.localStorage.setItem(MECHANICAL_DYNAMICS_EXPECTED_EXAM_STORAGE_KEY, JSON.stringify(snapshot)); } catch { /* Continue without storage. */ }
    }, 250);
    return () => window.clearTimeout(timer);
  }, [answers, awardedPoints, exam.id, hydrated, phase, practiceMinutes, remainingSeconds]);

  useEffect(() => () => document.body.classList.remove("mechanical-dynamics-expected-printing"), []);
  ...
  function startExam(examId: string) {
    ...
    window.setTimeout(() => document.getElementById("mechanical-dynamics-expected-paper")?.scrollIntoView({ behavior: "smooth", block: "start" }), 0);
  }
  ...
  function deleteSavedExam() {
    try { window.localStorage.removeItem(MECHANICAL_DYNAMICS_EXPECTED_EXAM_STORAGE_KEY); } catch { /* Ignore unavailable storage. */ }
    setSavedExam(null);
  ...
  function printExam(includeAnswers: boolean) {
    ...
    const cleanup = () => document.body.classList.remove("mechanical-dynamics-expected-printing");
    document.body.classList.add("mechanical-dynamics-expected-printing");
  ...
  if (phase === "setup") {
    return (
      <section className={styles.selector} id="mechanical-dynamics-expected-selector" aria-labelledby="mechanical-dynamics-expected-title">
        <header className={styles.selectorHeader}>
          <div><span>ACTUAL + EXPECTED EXAM · A4</span><h2 id="mechanical-dynamics-expected-title">機械力学 過去問・想定試験</h2></div>
          <p>過去問の7大問・100点・配点は確認済みです。試験時間だけ資料に記載がないため自分で設定し、60点ラインで確認します。</p>
        </header>
        <div className={styles.introStats} aria-label="想定試験の練習条件">
          <div><span>SETS</span><strong>{EXAMS.length}</strong><small>全範囲</small></div>
          <label><span>TIME</span><input type="number" min="1" max="180" inputMode="numeric" value={practiceMinutesDraft} onChange={(event) => setPracticeMinutesDraft(event.target.value)} /><small>分・自由設定</small></label>
          <div><span>SCORE</span><strong>100</strong><small>点換算</small></div>
          <div><span>PASS</span><strong>60</strong><small>練習用</small></div>
        </div>
        <aside className={styles.scopeWarning}><strong>出題範囲</strong><p>範囲ZIP15枚と機械力学過去問PDFの全内容を出題します。実物過去問と、同じ7大問・100点構成の想定試験を収録しています。</p></aside>
        ...
        <div className={styles.examGrid}>
          {EXAMS.map((candidate) => (
            <article className={styles.examCard} key={candidate.id}>
              <span>{isActualExam(candidate) ? "ACTUAL / 2025" : `${String(candidate.number).padStart(2, "0")} / ${String(MECHANICAL_DYNAMICS_EXPECTED_EXAMS.length).padStart(2, "0")}`}</span>
              <h3>{candidate.title}</h3><p>{candidate.subtitle ?? "機械力学・全範囲練習"}</p>
              <dl><div><dt>大問</dt><dd>{candidate.sections.length}</dd></div><div><dt>採点項目</dt><dd>{questionsFor(candidate).length}</dd></div><div><dt>{isActualExam(candidate) ? "公式配点" : "練習配点"}</dt><dd>{pointsFor(candidate)}</dd></div></dl>
              <small>{candidate.sections.map((section) => section.title).join("／")}</small>
              <button type="button" onClick={() => startExam(candidate.id)}>{isActualExam(candidate) ? "実物過去問を始める" : "この想定試験を始める"} →</button>
            </article>
          ))}
        </div>
  ...
  return (
    <section className={`${styles.lab} ${printAnswers ? styles.printAnswers : styles.printQuestions}`} aria-labelledby="mechanical-dynamics-active-title">
      ...
      {phase === "result" && (
        <div className={`${styles.scoreStrip} ${convertedScore >= PASS_PERCENT ? styles.pass : styles.fail}`}>
          <div><span>{actualExam ? "公式配点での得点" : "練習得点"}</span><strong>{score}<small>/{examPoints}</small></strong></div>
          <div><span>100点換算</span><strong>{convertedScore}<small>/100</small></strong></div>
          <p>{convertedScore >= PASS_PERCENT ? "60点ラインをクリア" : `60点まであと${PASS_PERCENT - convertedScore}点`}<small>{actualExam ? "実物過去問の公式100点配点です。" : "想定試験の練習用100点配点です。"}途中式・単位・線図の部分点は下で調整できます。</small></p>
        </div>
      )}

      <div className={styles.paperStack} id="mechanical-dynamics-expected-paper">
        {pages.map((pageSections, pageIndex) => (
          <article className={`${styles.paperPage} ${styles.questionPaperPage}`} key={`${exam.id}-page-${pageIndex + 1}`}>
            <header className={styles.paperHeader}><div><span>MECHANICAL DYNAMICS · PRACTICE PAPER</span><h1 id={pageIndex === 0 ? "mechanical-dynamics-active-title" : undefined}>{exam.title}</h1><p>{exam.subtitle ?? "範囲資料準拠・練習用想定問題"}</p></div><label><span>学生番号・氏名</span><input type="text" aria-label={`学生番号・氏名・${pageIndex + 1}ページ`} /></label></header>
            <div className={styles.paperMeta}><span>練習時間 {practiceMinutes}分</span><span>{actualExam ? "公式配点" : "練習配点"} {examPoints}点</span><span>60点ライン</span><span>PAGE {pageIndex + 1} / {paperPageCount}</span></div>
            {pageIndex === 0 && <p className={styles.instructions}>関数電卓使用可。途中式と単位を必ず記入し、分数・√・πのまま終えず最終数値を有効数字3桁で示すこと。π=3.14、g=9.80 m/s²とする。時間は練習用設定である。</p>}
            {pageSections.map((section) => (
              <section className={styles.examSection} key={`${exam.id}-section-${section.number}`}>
                <header className={styles.sectionHeader}><span>第{section.number}問</span><div><h2>{section.title}</h2><small>{section.topicIds?.join(" / ") ?? "MECHANICAL DYNAMICS"}</small></div><strong>{section.points}点</strong></header>
            ...
            <footer className={styles.paperFooter}><span>TEST//GRID · RANGE ZIP + ACTUAL EXAM</span><strong>{exam.id.toUpperCase()}</strong><span>PAGE {pageIndex + 1} / {paperPageCount}</span></footer>
          </article>
        ))}
        <article className={`${styles.paperPage} ${styles.answerSheetPage}`}>
          <header className={styles.paperHeader}><div><span>MECHANICAL DYNAMICS · ANSWER SHEET</span><h1>専用解答用紙</h1><p>{exam.title}・途中式と単位を必ず記入</p></div><label><span>学生番号・氏名</span><input type="text" aria-label="解答用紙の学生番号・氏名" /></label></header>
          <div className={styles.paperMeta}><span>関数電卓使用可</span><span>有効数字3桁</span><span>π=3.14・g=9.80 m/s²</span><span>PAGE {paperPageCount} / {paperPageCount}</span></div>
          <div className={styles.answerSheetGrid}>
            {exam.sections.map((section) => (
              <section className={styles.answerSheetItem} key={`answer-sheet-${exam.id}-${section.number}`}>
                <header><strong>第{section.number}問</strong><span>〔{section.points}点〕</span></header>
                {section.questions.map((question) => (
                  <label key={`answer-sheet-${question.id}`}><span>{section.questions.length > 1 ? `（${question.sub}）` : "解答"}</span><textarea rows={section.number === 6 ? 3 : 4} disabled={phase !== "active"} value={answers[question.id] ?? ""} onChange={(event) => setAnswers((current) => ({ ...current, [question.id]: event.target.value }))} placeholder={phase === "active" ? "式・数値・単位" : "中断中または採点済み"} /></label>
                ))}
              </section>
            ))}
          </div>
          <footer className={styles.paperFooter}><span>TEST//GRID · SEPARATE ANSWER SHEET</span><strong>{exam.id.toUpperCase()}</strong><span>PAGE {paperPageCount} / {paperPageCount}</span></footer>
        </article>
      </div>
      ...
      {phase === "result" && (
        <section className={styles.answerBook} aria-labelledby="mechanical-dynamics-solutions-title">
          <header className={styles.resultHeader}><div><span>ANSWER · UNITS · STEPS · SOURCE</span><h2 id="mechanical-dynamics-solutions-title">模範解答と途中式</h2><p>完全一致・数値・キーワードは仮採点済みです。同値な式、単位、有効数字、線図の部分点を各小問で調整してください。</p></div><strong>{convertedScore >= PASS_PERCENT ? "PASS" : "RETRY"}</strong></header>
                ...
                {section.questions.map((question) => {
                  ...
                  return (
                    <article className={styles.solutionCard} key={`solution-${question.id}`}>
                      ...
                      {question.diagram && <MechanicalDynamicsDiagram className={styles.diagram} kind={question.diagram} solution title={`${question.genre}：模範線図`} />}
...
export default function MechanicalDynamicsExpectedExams() {
  const firstExam = EXAMS[0];
  if (!firstExam) return <section className={styles.selector}><h2>想定試験を準備中です</h2><p>範囲分析が確定すると、ここへ全範囲セットが追加されます。</p></section>;
  return <MechanicalDynamicsExpectedExamLab firstExam={firstExam} />;
}
