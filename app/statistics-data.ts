...
const CORE_STATISTICS_QUESTIONS: StatisticsQuestion[] = [
  ...
  question({ id: "stats-q-bayes-machine", topic: "conditional", genre: "ベイズ", difficulty: 3, format: "number", prompt: "製品の60%を機械A、40%を機械Bが作り、Aの不良率は2%、Bの不良率は5%である。不良品が機械A製である確率を求めよ。小数第3位まで可。", answer: "\\(0.375=37.5\\%\\)", numericAnswer: 0.375, steps: ["Aかつ不良の確率は \\(0.6\\times0.02=0.012\\)", "不良全体の確率は \\(0.032\\)", "\\(P(A\\mid D)=\\frac{0.012}{0.032}=0.375\\)"], explanation: "分子は目的のA経路、分母は不良になる全経路。", formula: "P(A\\mid D)=\\frac{P(D\\mid A)P(A)}{P(D)}" }),
