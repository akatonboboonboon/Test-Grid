...
export const MATERIAL_MECHANICS_RANGE_PAGES: MaterialMechanicsRangePage[] = [
  ...
  { number: 14, filename: RANGE_FILENAMES[13], topics: ["beam-statics"], summary: "範囲補足の計算プリント。追加範囲4ケースの荷重・寸法・断面と途中計算を再確認する。", orientation: "portrait-source" },
  { number: 15, filename: RANGE_FILENAMES[14], topics: ["beam-statics"], summary: "範囲補足の用語プリント。指定された問2・問5・問7（文章化）・問10（文章変更）のみを対象とする。", orientation: "portrait-source" },
];
...
export const MATERIAL_MECHANICS_TOPICS: MaterialMechanicsTopic[] = [
  ...
  { id: "beam-statics", number: "04", title: "はりの反力・SFD・BMD・曲げ応力", shortTitle: "はり", description: "真直ばり、回転支点、集中荷重、片持ちばり、反力、せん断力図、曲げモーメント図、断面量と曲げ応力を扱う。", pages: [5, 6, 7, 8, 10, 11, 12, 13, 14, 15], color: "#63d8cb" },
];

type FormulaSeed = Omit<MaterialMechanicsFormulaCard, "sourceRefs"> & { pages: number[] };
const FORMULA_SEEDS: FormulaSeed[] = [
  { id: "mm-f-straight-beam", topic: "beam-statics", title: "真直ばり", prompt: "真直ばりとはどのようなはりか？", formula: "\\text{軸がまっすぐなはり}", explanation: "はりの中心軸が直線である部材を真直ばりという。補足プリント問2の指定範囲。", cue: "真直＝中心軸が直線", pages: [15] },
  { id: "mm-f-pin-support", topic: "beam-statics", title: "回転支点", prompt: "回転支点が拘束する運動と、持つ反力成分は？", formula: "\\begin{aligned}\\text{並進}&:\\text{拘束}\\\\ \\text{回転}&:\\text{自由}\\\\ \\text{反力}&:R_x,\\ R_y\\end{aligned}", explanation: "回転支点は部材の移動を止めるが回転は許す。したがって水平・鉛直反力を持ち、反力モーメントは持たない。", cue: "移動不可・回転可・モーメントなし", diagram: "support-types", pages: [15] },
  { id: "mm-f-concentrated-load", topic: "beam-statics", title: "集中荷重", prompt: "集中荷重Wを文章で説明し、単位を書け。", formula: "W\\ [\\mathrm{N}]", explanation: "非常に狭い範囲へ作用する荷重を、ある一点に集約して表したものが集中荷重で、単位はNである。補足プリント問7は文章で答える。", cue: "一点に集約・単位N", pages: [15] },
  { id: "mm-f-cantilever-definition", topic: "beam-statics", title: "片持ちばりの定義", prompt: "片持ちばりを支持条件から文章で説明せよ。", formula: "\\text{一端固定・他端自由}", explanation: "一端を固定端で完全に拘束し、他端を自由端としたはりを片持ちばりという。固定端には水平・鉛直反力と反力モーメントが生じ得る。", cue: "一端固定、他端自由", diagram: "cantilever-udl", pages: [15] },
  { id: "mm-f-bending-stress", topic: "beam-statics", title: "曲げ応力の基本式", prompt: "中立軸から距離yの曲げ応力と、最外縁の最大曲げ応力は？", formula: "\\sigma=\\frac{My}{I}=\\frac{M}{Z}", explanation: "曲げ応力は中立軸で0、最外縁で最大となる。Iは曲げに対する断面二次モーメント、Z=I/cは断面係数であり、ねじりの極断面量Ip・Zpとは区別する。", cue: "曲げはIとZ（Ip・Zpではない）", diagram: "beam-section-stress", pages: [10, 11, 12, 13, 14] },
...
export const MATERIAL_MECHANICS_QUESTIONS: MaterialMechanicsQuestion[] = [
  question({ id: "mm-q-supplement-q2-straight-beam", topic: "beam-statics", genre: "補足プリント問2・用語", difficulty: 1, format: "text", prompt: "真直ばりとは何か、中心軸に着目して文章で説明せよ。", answer: "中心軸がまっすぐなはり。", accepted: ["軸がまっすぐなはり", "中心軸が直線のはり"], keywords: ["軸", "まっすぐ"], minKeywords: 2, formula: "\\text{真直ばり}=\\text{中心軸が直線のはり}", steps: ["はりの形状ではなく中心軸を見る。", "中心軸が曲線ではなく直線であることを文章にする。"], explanation: "材力範囲プリント2の指定問2。空欄名だけでなく『中心軸がまっすぐ』まで書けるようにする。", pages: [15] }),
  question({ id: "mm-q-supplement-q5-pin-support", topic: "beam-statics", genre: "補足プリント問5・回転支点", difficulty: 2, format: "text", prompt: "回転支点について、移動・回転の可否、発生する反力成分、反力モーメントの有無を文章で説明せよ。", answer: "回転支点は部材の移動を拘束するが回転は許す。水平・鉛直の2反力を持ち、反力モーメントは生じない。", keywords: ["移動", "回転", "水平", "鉛直", "モーメント"], minKeywords: 5, formula: "\\begin{aligned}R_x&\\ne0,\\quad R_y\\ne0\\\\ M_R&=0\\end{aligned}", steps: ["並進2自由度と回転1自由度を分ける。", "拘束された並進に対応して水平・鉛直反力を置く。", "回転は自由なので反力モーメントを置かない。"], explanation: "材力範囲プリント2の指定問5。斜め荷重は水平・鉛直へ分解して、この2反力でつり合わせる。", diagram: "support-types", pages: [15] }),
  question({ id: "mm-q-supplement-q7-concentrated-load", topic: "beam-statics", genre: "補足プリント問7・文章化", difficulty: 1, format: "text", prompt: "集中荷重Wとはどのような荷重か。図の一点に作用する表現と単位を含め、文章で説明せよ。", answer: "集中荷重Wは、狭い範囲へ加わる荷重を一点に集約して表した荷重で、単位はNである。", keywords: ["一点", "荷重", "N"], minKeywords: 3, formula: "W\\ [\\mathrm{N}]", steps: ["分布荷重w[N/m]との違いを意識する。", "一点に集約して表すことと、力の単位Nを書く。"], explanation: "材力範囲プリント2の指定問7は空欄ではなく文章化して出るため、名称・意味・単位を一文で答える。", diagram: "load-resultants", pages: [15] }),
  question({ id: "mm-q-supplement-q10-cantilever-wording", topic: "beam-statics", genre: "補足プリント問10・文章変更", difficulty: 2, format: "text", prompt: "一端を完全に固定し、他端を自由としたはりの名称を答え、固定端に生じ得る反力を文章で説明せよ。", answer: "片持ちばり。固定端には水平反力・鉛直反力・反力モーメントが生じ得る。", keywords: ["片持ち", "水平", "鉛直", "モーメント"], minKeywords: 4, formula: "\\text{固定端}:R_x,\\ R_y,\\ M_0", steps: ["『一端固定・他端自由』から片持ちばりと判断する。", "固定端は並進と回転をすべて拘束するため、2反力と1反力モーメントを置く。"], explanation: "材力範囲プリント2の指定問10は元の単語穴埋めから文章を変えて出るため、支持条件と固定端反力まで答える。", diagram: "cantilever-udl", pages: [15] }),
  question({ id: "mm-q-add-c3-reactions", topic: "beam-statics", genre: "追加例題3・固定端反力", difficulty: 2, format: "text", context: "長さL=2.0 mの片持ちばり自由端にP=4.0 kNが下向きに作用する。断面は外径do=100 mm、内径di=80 mmの中空円。", prompt: "固定端の鉛直反力Rと反力モーメントの大きさ|M0|を求めよ。", answer: "\\(R=4.0\\,\\mathrm{kN},\\;|M_0|=8.0\\,\\mathrm{kN\\,m}\\)", keywords: ["4.0", "8.0", "上向き"], minKeywords: 2, formula: "R=P,\\quad |M_0|=PL", steps: ["鉛直力のつり合いから固定端反力は荷重と逆向きで\\(R=P=4.0\\,\\mathrm{kN}\\)。", "荷重Pの固定端まわり腕長はL=2.0 m。", "\\(|M_0|=PL=4.0(2.0)=8.0\\,\\mathrm{kN m}\\)。反力モーメントは荷重の回転を打ち消す向き。"], explanation: "固定端は力反力とモーメント反力の両方を持つ。", diagram: "additional-cantilever-tip-hollow", pages: [10, 11, 12, 13, 14] }),
...
function buildBeamExamSection(beam: BeamExamCase, examPrefix: string): MaterialMechanicsExamSection {
  ...
  return {
    ...
    questions: [
      examQuestion({ id: `${examPrefix}-5-1`, major: 5, sub: 1, points: 7, topic: "beam-statics", genre: "反力", difficulty: 2, format: "text", prompt: isSimple ? "支点反力RA、RBを求めよ。" : "固定端鉛直反力Rと反力モーメントの大きさ|M0|を求めよ。", answer: reactionAnswer, keywords: [fixed(reactionA, 3).toString(), fixed(isSimple ? reactionB : maxMoment, 3).toString()], minKeywords: 2, formula: reactionFormula, steps: reactionSteps, explanation: "反力を次問のSFD/BMDへ引き継ぐ。力とモーメントのつり合いを単位付きで確認する。", diagram: beam.diagram, pages: [10, 11, 12, 13, 14] }),
      examQuestion({ id: `${examPrefix}-5-2`, major: 5, sub: 2, points: 7, topic: "beam-statics", genre: "SFD/BMDと最大曲げモーメント", difficulty: 3, format: "diagram", prompt: "前問の反力を用いてV(x)、M(x)を示し、SFD/BMDを描いて最大曲げモーメントの大きさを求めよ。", answer: `\\(|M_{max}|=${fixed(maxMoment, 3)}\\,\\mathrm{kN\\,m}\\)。式と図の形は解答手順のとおり。`, keywords: ["V", "M", fixed(maxMoment, 3).toString()], minKeywords: 3, dependsOn: [`${examPrefix}-5-1`], formula: vmFormula, steps: vmSteps, explanation: "荷重状態に応じたV・Mを作り、最大曲げモーメントを次問へ引き継ぐ。", diagram: beam.diagram, pages: [10, 11, 12, 13, 14] }),
      examQuestion({ id: `${examPrefix}-5-3`, major: 5, sub: 3, points: 6, topic: "beam-statics", genre: "最大曲げ応力", difficulty: 3, format: "number", prompt: "前問の|Mmax|と断面係数Zを用いて、最外縁の最大曲げ応力の大きさを求めよ。", answer: `${fixed(stress, 2)} MPa`, numericAnswer: fixed(stress, 5), expectedUnit: "MPa", acceptedUnits: stressUnits, requiresUnit: true, tolerance: Math.max(0.1, stress * 0.005), dependsOn: [`${examPrefix}-5-2`], formula: sectionFormula, steps: sectionSteps, explanation: "曲げのI・Zを使う。極断面二次モーメントIp・極断面係数Zpへの取り違えに注意する。", diagram: beam.diagram, pages: [10, 11, 12, 13, 14] }),
    ],
  };
}
function buildExpectedExam(variant: Variant, index: number): MaterialMechanicsExam {
  ...
  return {
    ...
    subtitle: "範囲資料15枚（ZIP13枚＋補足2枚）に準拠・50分練習",
    variant: index + 1,
    defaultMinutes: 50,
    userAdjustable: true,
    officialMinutes: null,
    ...
    passPercent: 60,
    paper: "A4 portrait",
    officialConditionsNote: "範囲資料15枚（ZIP13枚＋補足2枚）を正本とし、中実軸の応力・ねじり角、中空軸の動力・外径設計、密巻ばね、追加範囲4ケース、補足問2・5・7・10を出題する。過去問は紙面構成だけを参照し、数値・荷重配置・旧範囲内容は出題しない。",
    sections,
...
export const MATERIAL_MECHANICS_SUPPLEMENT_QUESTIONS: MaterialMechanicsExamQuestion[] = MATERIAL_MECHANICS_QUESTIONS
  .filter((question) => question.id.startsWith("mm-q-supplement-"))
  .map((question, index) => ({
    ...question,
    major: 5,
    sub: index + 1,
    points: 5,
    genre: `本番補足・${question.genre}`,
  }));
...
export const MATERIAL_MECHANICS_PRACTICE_QUESTIONS: MaterialMechanicsExamQuestion[] = [
  ...MATERIAL_MECHANICS_PRINT_LEVEL_QUESTIONS,
  ...MATERIAL_MECHANICS_SUPPLEMENT_QUESTIONS,
];
...
export const MATERIAL_MECHANICS_SOURCE_POLICY = {
  ...
  note: "範囲ZIP13枚と今回の補足2枚だけを問題内容の正本とする。補足プリント2は指定された問2・5・7・10以外を出題しない。過去問は出題の見た目・大問構成だけを参考にする。",
} as const;
