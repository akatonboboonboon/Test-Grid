...
export const MATERIAL_MECHANICS_QUESTIONS: MaterialMechanicsQuestion[] = [
  ...
  question({ id: "mm-q-overhang-format2", topic: "beam-statics", genre: "形式2・第2問", difficulty: 3, format: "text", context: "図でC-A=100 mm、A-D=200 mm、D-B=200 mm。Cに下向き300 N、Dに上向き150 N。", prompt: "支点反力RA、RBを向き付きで求めよ。", answer: "\\(R_A=300\\,\\mathrm{N}\\)上向き、\\(R_B=150\\,\\mathrm{N}\\)下向き", keywords: ["300", "150", "上向き", "下向き"], minKeywords: 4, formula: "\\sum F_y=0,\\quad \\sum M_A=0", steps: ["上向きを正として\\(R_A+R_B+150-300=0\\)。", "A回り（反時計回りを正）で\\(300(100)+150(200)+R_B(400)=0\\)。", "\\(R_B=-150\\,\\mathrm{N},\\;R_A=300\\,\\mathrm{N}\\)。"], explanation: "負のRBは仮定した上向きと逆、すなわち下向き反力を示す。形式2のうち今回範囲と一致する設問。", diagram: "overhang-beam", pages: [5, 6, 8], overlap: { question: 2, sub: 1 } }),
